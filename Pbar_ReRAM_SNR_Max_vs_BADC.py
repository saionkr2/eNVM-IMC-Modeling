...
for Bind in range(len(B_ADC)):
    ...
    for Vind in range(len(V_bl_all)):
        ...
        print('SNR optimal clipping at',zeta[np.argmin(Var_noi_tot[Bind,Vind,:])])
...
np.save('SNR_dB_ReRAM_vs_BADC.npy',SNR_dB)
#%% 
...
fig1 = plt.figure(figsize=(11,8))
ax1 = fig1.add_subplot(1,1,1)
ax1.plot(B_ADC, SNR_max,c="r", markersize=10, marker='o', linewidth=lin_w)
ax1.grid(1,'major', linewidth=0.5, color='black')
ax1.grid(1,'minor', linewidth=0.5, ls='--')
ax1.set_axisbelow(True)
plt.box(1)
#plt.savefig('ReRAM_SNR_max_vs_BADC.png', type='png', bbox_inches='tight')
