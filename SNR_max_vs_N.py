...
#%%
#Ideal = np.max(np.load('SNR_dB_ideal_vs_N.npy'),axis=1)
ReRAM = np.max(np.load('SNR_dB_ReRAM_vs_N.npy'),axis=1)
MRAM = np.max(np.load('SNR_dB_MRAM_vs_N.npy'),axis=1)
FeFET = np.max(np.load('SNR_dB_FeFET_vs_N.npy'),axis=1)
...
#ax1.plot(N_all*0.5, Ideal,c='green', markersize=10, marker='s', linewidth=lin_w,label='Ideal')
...
plt.box(1)
plt.savefig('SNR_max_vs_N.pdf', bbox_inches='tight',dpi=200)