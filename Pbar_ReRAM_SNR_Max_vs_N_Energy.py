...
#N_all = np.logspace(0.5,4,30)
a_vec_no = 61
w_vec_no = 61

Var_sig = np.zeros((len(N_all),len(V_bl_all)))

Avg_Geq = np.zeros((len(N_all),len(V_bl_all)))
...
for Vind in range(len(V_bl_all)):
    ...
    for Nind in range(len(N_all)):
        ...
        Avg_Geq[Nind,Vind] = np.mean(1/R_temp_wdg)
...
np.save('SNR_dB_ReRAM_vs_N_E1b.npy', SNR_dB)
np.save('AGeq_ReRAM_vs_N.npy', Avg_Geq)
#%% 
...
for i in range(len(N_all)):
    ax1.plot(V_bl_all, SNR_dB[i,:],c="r", markersize=10, marker='o', linewidth=lin_w)
...
fig1 = plt.figure(figsize=(11,8))
ax1 = fig1.add_subplot(1,1,1)
ax1.plot(N_all*0.5, SNR_max,c="r", markersize=10, marker='o', linewidth=lin_w)
ax1.grid(1,'major', linewidth=0.5, color='black')
ax1.grid(1,'minor', linewidth=0.5, ls='--')
plt.xscale("log")
ax1.set_axisbelow(True)
plt.box(1)
#plt.savefig('ReRAM_SNR_max_vs_N.pdf', bbox_inches='tight',dpi=200)
