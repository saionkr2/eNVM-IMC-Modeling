...
def Quantize(x,clpmax,clpmin,BADC):
    ...
    val = np.round(xclp/lim)*2-(2**BADC-2)
    return val


def Quantize_print(x,clpmax,clpmin,BADC):
    xclp = np.minimum(np.maximum(x,clpmin),clpmax) - clpmin
    #print(xclp)
    lim = (clpmax-clpmin)/(2**BADC)
    #print(lim)
    val = np.round(xclp/lim)*2-(2**BADC)
    print(np.round(xclp/lim),val)
    return val
...
for Vind in range(len(V_bl_all)):
    ...
    for Nind in range(len(N_all)):
        ...
        #print(V_bl_s*Nreal*g_on/m,np.max(Isig_wpar_wl_wm))
        #print(V_bl_s*Nreal*g_off/m,np.min(Isig_wpar_wl_wm))
        #print('max_sig',(np.max(Isig_wpar_wl_wm)-I_mu[Nind,Vind])/I_std[Nind,Vind])
        #print('min_sig',(np.min(Isig_wpar_wl_wm)-I_mu[Nind,Vind])/I_std[Nind,Vind])
        for Zind in range(zlen):
            ...
            IA_Clp_min = V_bl_s*N*g_off/m
            IA_Clp_max = V_bl_s*N*g_on/m
            ...
            #print(y_ideal)
            y_bev[Nind,Vind,Zind,:] = Quantize_print(Isig_wpar_wl_wm + Ith,IA_Clp_max,IA_Clp_min,B_ADC)
            yn[Nind,Vind,Zind,:] = y_bev[Nind,Vind,Zind,:] - y_ideal
            print(y_ideal[np.where(yn[Nind,Vind,Zind,:]!=0)],y_bev[Nind,Vind,Zind,np.where(yn[Nind,Vind,Zind,:]!=0)])
            #if(Vind ==24):
            #     print(zeta[Zind],I_lsb, I_mu[Nind,Vind], I_std[Nind,Vind])
            #     print(Isig_wpar_wl_wm[0] + Ith[0],IA_Clp_min,IA_Clp_max)
            #     print(Quantize_print(Isig_wpar_wl_wm[0] + Ith[0],IA_Clp_max,IA_Clp_min,B_ADC),y_bev[Nind,Vind,Zind,0],y_ideal[0],yn[Nind,Vind,Zind,0])
...
#np.save('SNR_MRAM_vs_Vbl_CV_PC.npy',SNR)
#%% 
