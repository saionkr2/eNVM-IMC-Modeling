...
for Vind in range(len(V_bl_all)):
    ...
    for Nind in range(len(N_all)):
        ...
        ran_w_bin = np.random.randint(0,2,size=(N,w_vec_no))*2-1
        ...
        IA_Clp_min_FR = np.min(Isig_wpar_wl_wm)
        IA_Clp_max_FR = np.max(Isig_wpar_wl_wm)
...
#np.save('SNDRd_MRAM_vs_Vbl_All.npy',SNDRd_dB)
#%% 
...
plt.ylabel(r'$\mathrm{SNR}$ (dB)',fontsize=25,fontdict=font)
plt.xlabel(r'$V_{\mathrm{ref}}$ (mV)',fontsize=25,fontdict=font)

#plt.savefig('MRAM_SNDRd_vs_Vbl_plot.pdf', bbox_inches='tight', dpi =200)   
