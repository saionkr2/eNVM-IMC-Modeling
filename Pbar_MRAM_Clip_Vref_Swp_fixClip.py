...
yn = np.zeros((len(N_all),len(V_bl_all),a_vec_no*w_vec_no))
y_bev = np.zeros((len(N_all),len(V_bl_all),a_vec_no*w_vec_no))
...
for Vind in range(len(V_bl_all)):
    ...
    for Nind in range(len(N_all)):
        ...
        #IA_Clp_min = (V_bl_s*1/np.max(R_temp_wdg))/m
        #IA_Clp_max = (V_bl_s*1/np.min(R_temp_wdg))/m
        #print((IA_Clp_max - IA_Clp_min)/(2*I_std[Nind,Vind]))
        I_lsb = 2*zeta*I_std[Nind,Vind]/(2**B_ADC-2)
        IA_Clp_min = I_mu[Nind,Vind] - I_lsb*(2**B_ADC-2)/2
        IA_Clp_max = I_mu[Nind,Vind] + I_lsb*(2**B_ADC-2)/2
        #Sweep to find the clipping range
        y_bev[Nind,Vind,:] = np.round((Quantize(Isig_wpar_wl_wm + Ith,IA_Clp_max,IA_Clp_min,B_ADC)/2**B_ADC)*(zeta*np.std(y_ideal)))
        yn[Nind,Vind,:] = y_bev[Nind,Vind,:] - y_ideal
            
        Var_noi_tot[Nind,Vind] = np.var(yn[Nind,Vind,:])
        print(np.var(y_ideal),np.var(y_bev[Nind,Vind,:]))   
...
ax.set_ylabel(r"Behavioral" "\n" "Dot-Product", fontsize = 32)
sort_idx = np.argsort(y_ideal)
ax.plot(y_ideal[sort_idx], y_bev[0,-1,sort_idx], color='black',linewidth=4,label=r'ideal ($y_o$)')
