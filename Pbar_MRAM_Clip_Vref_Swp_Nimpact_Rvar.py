...
for Vind in range(len(V_bl_all)):
    ...
    for Nind in range(len(N_all)):
        ...
        r_vec = np.zeros([N,1])
        del_r_vec = np.zeros([N,a_vec_no])
        
        for i in range(int(N/2)):
            if(ran_w_vec[i]==-1):
                r_vec[2*i,0] = R_AP
                r_vec[2*i+1,0] = R_P
                del_r_vec[2*i,:] = Sig_mb_gap*np.random.normal(size=(a_vec_no))
                del_r_vec[2*i+1,:] = Sig_mb_gp*np.random.normal(size=(a_vec_no))
            else:
                r_vec[2*i,0] = R_P
                r_vec[2*i+1,0] = R_AP
                del_r_vec[2*i,:] = Sig_mb_gp*np.random.normal(size=(a_vec_no))
                del_r_vec[2*i+1,:] = Sig_mb_gap*np.random.normal(size=(a_vec_no))
        ...
        y_mult_wdg = np.multiply(r_vec+del_r_vec,a_mat_01).T
        ...
        Nreal = N/2
        Isig =  V_bl_s*np.sum(a_mat_01*1/r_vec,0)/m
...
#np.save('SNR_MRAM_vs_Vbl_only_quant_clip.npy',SNR)
#%% 
