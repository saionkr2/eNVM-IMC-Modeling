...
#Ideal = np.max(np.load('SNR_dB_ideal_vs_BADC.npy'),axis=1)
ReRAM = np.max(np.load('SNR_dB_ReRAM_vs_BADC.npy'),axis=1)
MRAM = np.max(np.load('SNR_dB_MRAM_vs_BADC.npy'),axis=1)
FeFET = np.max(np.load('SNR_dB_FeFET_vs_BADC.npy'),axis=1)
...
plt.box(1)
plt.savefig('SNR_max_vs_BADC.pdf', bbox_inches='tight',dpi=200)